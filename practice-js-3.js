...
var yourName = 'Michelle'

//////////////////STEP 3////////////////////
/*
    Create a function called 'setGroceryListTitle' that takes in one parameter called 'x'. 
    The function should return a string of 'APARAMETER's Grocery List'.
...
function setGroceryListTitle (x){
    return x + "'s Grocery List"; //or return `${x}'s Grocery List`
}
...
function addItem (item){
    //Put item at the end of groceries
    //[1,2,3]
    groceries.push(item); //array_name.push(item that we need to add)
    //[1,2,3,4]
    //Invoke displayData() function
    displayData();
}
...
function checkGroceryCount (){
    if (groceries.length > 5) {
        ...
        return groceries.length + ' items'; //3 Items
    }
    else {
